...
impl GapFill {
    pub fn new(regular: &DataFrame, extended: &DataFrame) -> Self {
        ...
        Self {
            pre_market_high: f64::max(pre_market_high, prev_high),
            prev_high: f64::min(pre_market_high, prev_high),
            prev_low: f64::max(prev_low, pre_market_low),
            pre_market_low: f64::min(prev_low, pre_market_low),
        }
    }
    ...
    pub fn check_sr(&self, trend: Trend, candles: &[Candle], atr: f64) -> Option<PriceRejection> {
        ...
        if trend == Trend::Bullish
            ...
            && last.close < self.pre_market_high
        {
            check_support(candles, self.prev_high, atr)
        } else if trend == Trend::Bearish
            && self.prev_low >= last.close
            && last.close > self.pre_market_low
        {
